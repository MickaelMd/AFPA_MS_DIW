...
if (isset($_POST['profil_submit'])) {
    ...
    if (!empty($_POST['profil_telephone']) && !preg_match("/^(\+?\d{1,3}\s?)?([1-9](\s?\d\s?){8}|\d{10,14})$/", $_POST['profil_telephone'])) {
        ...
        if (!empty($_POST['profil_adresse']) && !preg_match("/^\d+\s[A-Za-zÀ-ÿ0-9\s.'-]+(?:\sAppartement\s\d+)?\s*,?\s*\d{5}\s[A-Za-zÀ-ÿ\s.'-]+$/", $_POST['profil_adresse'])) {
            ...
            $fieldsToUpdate = [];
            if (!empty($_POST['profil_nom'])) $fieldsToUpdate['nom'] = $_POST['profil_nom'];
            if (!empty($_POST['profil_prenom'])) $fieldsToUpdate['prenom'] = $_POST['profil_prenom'];
            if (!empty($_POST['profil_email'])) $fieldsToUpdate['email'] = $_POST['profil_email'];
            if (!empty($_POST['profil_telephone'])) $fieldsToUpdate['telephone'] = $_POST['profil_telephone'];
            if (!empty($_POST['profil_adresse'])) $fieldsToUpdate['adresse'] = $_POST['profil_adresse'];
            if (!empty($_POST['profil_pwd'])) {
